...
export class TeamRetrieval {
  ...
  constructor (private playerService : PlayerRetrieval) {}
    ...
    private updateTeam(t: Team) : void{
      ...
      console.error ('Something went terribly wrong running private updateTeam on '  + t.name);
    }
    ...
    private getPlayersOfTeam (m : Match, t : Team) : Array<Player>{
      const tempPlayerList : Array<Player> = [];
    ...
    addUpdateTeam (m : Match, t : Team) : void{
      ...
      if(this.teamExists(t)){
        ...
        console.error('Something went wrong in addUpdateTeam for teams '
            + t.name);
      }
